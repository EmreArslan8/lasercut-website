  ...
  try {
    ...
    if (!dxfData || !dxfData.entities || dxfData.entities.length === 0) {
      return NextResponse.json(
        { error: "Geçerli DXF verisi bulunamadı" },
        { status: 400 }
      );
    }
    ...
    svg = svg
      ...
      .replace(
        /<path /g,
        '<path stroke="black" stroke-width="1" fill="none" stroke-linecap="round" stroke-linejoin="round" '
      )
      .replace(
        /<polygon /g,
        '<polygon stroke="black" stroke-width="1" fill="lightgray" '
      )
      .replace(
        /<rect /g,
        '<rect stroke="black" stroke-width="1" fill="lightgray" '
      )
      .replace(
        /<circle /g,
    ...
    return NextResponse.json(
      { error: "DXF işleme hatası", details: (error as Error).message },
      { status: 500 }
    );
  }
}
