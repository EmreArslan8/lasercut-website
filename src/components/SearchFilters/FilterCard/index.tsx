  ...
  return (
    ...
    >
      {type === 'category' && data.length === 1 && (
        ...
        >
          {t('filters.backToPreviousCategories')}
        </Button>
      )}
      {scrollable && (
        <TextField
          ...
          placeholder={t('filters.searchPlaceholder', { type })}
          ...
          <Grid container columnSpacing={2}>
            {options.length ? (
              options.map((e) => (
                <Grid item xs={6} sm={12} key={JSON.stringify(e.searchOptions) + e.text}>
                  ...
                  >
                    ...
                    {type === 'gender' ? t(`filters.genders.${e.text}`) : e.text}
                  </Stack>
                </Grid>
              ))
            ) : (
              <Stack textAlign="center" mt={1}>
                ...
                <Typography variant="warning">{t('filters.noOptionsFound')}</Typography>
              </Stack>
            )}
          </Grid>
        </Stack>
