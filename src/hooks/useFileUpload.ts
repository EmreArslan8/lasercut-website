...
const useFileUpload = (): UseFileUploadReturn => {
  ...
  const uploadFiles = async (
    ...
    for (const file of files) {
      try {
        ...
        console.log(
          `📂 Dosya yüklendi: ${file.name} (Uzantı: ${fileExtension})`
        );
        ...
        if (!allowedExtensions.includes(fileExtension!)) {
          ...
          results.push({
            success: false,
            message: `Geçersiz dosya türü: ${file.name}`,
          });
          continue;
        }
        ...
        if (uploadError) {
          ...
          results.push({
            success: false,
            message: `Yükleme hatası: ${uploadError.message}`,
          });
          continue;
        }
        ...
        if (dbError) {
          ...
          results.push({
            success: false,
            message: `DB hatası: ${dbError.message}`,
          });
          continue;
        }
        ...
        results.push({
          success: true,
          message: `✅ Dosya başarıyla yüklendi: ${sanitizedFileName}`,
        });
      } catch (error) {
  ...
  const handleFileUpload = (files: File[]) => {
    ...
    // DXF kontrolü
    const hasDxf = validFiles.some((file) =>
      file.name.toLowerCase().endsWith(".dxf")
    );
    setIsDxf(hasDxf);

    console.log(
      `🔍 DXF kontrolü: ${hasDxf ? "Evet, DXF dosyası var." : "Hayır, DXF dosyası yok."}`
    );
  };
